...
function CochePolicia(contexto,contextoUI,mapa){
	...
	_this.dinero = 3000;
		...
		_this.algoritmoBuscaCoche = function(){
			...
			_this.rotation = {rotation:_this.direccion};
			_this.direccion = _this.hallarAngulo(_this.pixelX[_this.indiceActual],_this.pixelY[_this.indiceActual],_this.pixelX[_this.indiceGanador],_this.pixelY[_this.indiceGanador]);
			_this.tween = new TWEEN.Tween(_this.position)
				.to({x: _this.pixelX[_this.indiceGanador], y: _this.pixelY[_this.indiceGanador]}, 900)
				//.delay(10)
			...
			_this.tweenrotation = new TWEEN.Tween(_this.rotation)
				.to({rotation:_this.direccion}, 500)
				//.delay(10)
				.easing(TWEEN.Easing.Circular.Out)
				.onUpdate(_this.mueve);
				
			_this.tweenrotation.start();
		...
		_this.mueve = function(){
				contextoFinalPolicias.clearRect(0,0,7552,4224);
				//console.log(_this.rotation.rotation);
				if(_this.direccion == -90){
					
					_this.drawRotated(Math.radians(270));
					
				}else if(_this.direccion == 180){
					_this.drawRotated(Math.radians(-180));
				
				}else{
					_this.drawRotated(Math.radians(_this.rotation.rotation));
					
				}
				//_this.drawRotated(Math.radians(_this.rotation.rotation));
				_this.render();
				if(_this.contadorParaBuscar % 10 == 0){
					_this.buscaColor();
				}
		...
		_this.drawRotated = function(radianes){
			contextoFinalPolicias.clearRect(0,0,7552,4224);
			contextoFinalPolicias.save();
			contextoFinalPolicias.translate(_this.position.x-camara.vistaX, _this.position.y-camara.vistaY);
			contextoFinalPolicias.rotate(radianes);
			contextoFinalPolicias.drawImage(cochePolicia, -cochePolicia.width / 2, -cochePolicia.height / 2);
			contextoFinalPolicias.restore();
		};
